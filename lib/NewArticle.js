...
export default React.createClass({
  ...
  componentWillMount() {
    $.ajax({
      type: "GET",
      url: "http://tiny-tiny.herokuapp.com/collections/sa_tech_userdata_test",
      contentType: "application/json",
      success: (response) => {
        this.setState({
          blogs: {
            user_info: {
              user_author: response[0].name,
              user_imgSrc: response[0].picture,
  ...
  onSubmitHandler(e){
    ...
    var userInfoObject = {
      ...
      user_name: this.state.user_author,
      user_imgSrc: this.state.user_imgSrc
    }
